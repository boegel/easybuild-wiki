...
There are very little limitations to implementing a custom module naming scheme. An earlier limitation involving only the `name`, `version`, `versionsuffix` and `toolchain` easyconfig parameters to be available was resolved with EasyBuild v1.14.0 (see [easybuild-framework issue #687](https://github.com/hpcugent/easybuild-framework/issues/687)).
